...
turtles_aq_act_contrasts <- function(fx) {
  ...
  ggplot(fx) + aes(y = effect) +
    facet_grid(contrast ~ ., scales = "free_y", space = "free_y") +
    geom_vline(
      color = opts$gry, size = opts$lwd,
...
boxplot_action_acc <- function() {
  dt <- copy_dt() %>%
    .[
      , .(acc = mean(recog_acc)),
      .(id, action, aq_category, action_category)
    ] %>%
    .[, mu := mean(acc), action] %>%
    .[order(action_category, -mu), ] %>%
    .[
      ,
      action := factor(action, levels = as.character(
        unique(action)
      ))
    ] %>%
    copy_dt(TRUE)
  ...
  ggplot(dt) + aes(x = action, y = acc) +
    geom_boxplot(aes(fill = Actions, colour = Actions),
      size = opts$lwd, width = 0.5, outlier.alpha = 0,
      outlier.size = 1, notch = FALSE, show.legend = FALSE) +
    geom_point(
      data = outliers, aes(fill = Actions, colour = Actions),
      alpha = 0.5, size = 0.5, show.legend = FALSE,
      position = position_jitter(0.25, 0.02)) +
    scale_fill_manual(values = opts$c_pal[c(1, 5)]) +
    scale_colour_manual(values = opts$c_pal[c(2, 4)]) +
    coord_flip(ylim = c(0, 1)) +
    facet_grid(Actions ~ ., space = "free", scales = "free") +
...
boxplot_aq_action_acc <- function() {
  dt <- copy_dt() %>%
    .[
      , .(acc = mean(recog_acc)),
      .(id, aq_category, action_category)
    ] %>%
  ...
  ggplot(dt) + aes(x = AQ, y = acc) +
    geom_boxplot(
      ...
      size = opts$lwd,
      width = 0.333, position = position_dodge(width = 0.5),
      outlier.size = 0.7, outlier.stroke = 0.5,
      outlier.shape = 21, notch = FALSE, show.legend = TRUE
    ) +
    geom_point(aes(fill = Actions),
      shape = 20, alpha = 0.5,
      colour = opts$blk, show.legend = FALSE,
...
plot_action_post_acc <- function(fx) {
  ...
  ggplot(fx) + facet_wrap(~ action_category, scales = "free") +
    geom_vline(
      color = opts$gry, size = opts$lwd,
      linetype = 1, xintercept = 0.25) +
    ggdistribute::geom_posterior(
      aes(x = .y, y = action),
      size = 0.15, ci_width = 0.9,
      interval_type = "ci", center_stat = "mean", mirror = FALSE,
      adjust = 1.1, n = 1024, trim = .003, cut = 1
    ) +
    geom_point(
      data = acc_dta, aes(x = .y, y = action),
      color = opts$blk, position = position_nudge(y = -0.333),
      shape = 21, size = 1, fill = "#FFFFFF") +
    scale_x_continuous(breaks = seq(-1, 1, .1)) +
    coord_cartesian(xlim = c(.25, 1)) +
    labs(x = "Self-recognition accuracy") +
    theme(
      panel.border = element_rect(
        fill = NA, colour = opts$gry, size = 0.67
      ),
      axis.title.y = element_blank(),
...
hist_plot_acc <- function() {
  ...
  plt_sum <- copy_dt(NULL) %>%
    .[
      , .(.y = mean(recog_acc)),
      .(aq_category, action_category, id, action)
    ] %>%
    .[, count := .N, .(aq_category, action_category)] %>%
    .[
      , .(n_freq = .N / count),
      .(aq_category, action_category, .y, count)
    ] %>%
  ...
  ggplot(plt_sum) + aes(x = .y, y = n_freq) +
    geom_rect(aes(
      xmin = acc_range[1], xmax = acc_range[2],
      ymin = 0, ymax = 1), fill = grey(0.96)) +
    geom_vline(aes(xintercept = v),
      linetype = 2,
      size = opts$lwd, color = opts$blk) +
    geom_bar(aes(color = aq_category, fill = aq_category),
      width = 0.1, size = opts$lwd, stat = "identity") +
    geom_text(aes(x = 0, y = 0.5, label = boxes_txt),
      hjust = 0,
      vjust = 1, family = getOption("SRE.font_family")) +
    ...
    coord_cartesian(
      xlim = c(-0.1, 1.1),
      ylim = c(0, max(plt_sum$n_freq) + .05),
      expand = FALSE) +
    labs(
      x = "Self-recognition accuracy",
...
interaction_plot_bar <- function() {
  ...
  plt_summary_dat <- sre %>%
    ...
    .[, `:=`(
      aq_category = factor(
        aq_category,
        levels = c("low symptoms", "high symptoms"),
  ...
  ggplot(plt_summary_dat) + aes(action_category, y) +
    geom_hline(aes(yintercept = .25),
      color = opts$gry,
      size = 0.333, linetype = 1) +
    geom_bar(
      stat = "identity",
      position = position_dodge(width = opts$dodge_w), size = 1,
      color = NA, width = 0.67, aes(fill = aq_category)) +
    geom_linerange(
      stat = "identity",
      position = position_dodge(width = opts$dodge_w),
      size = 0.8, color = "white",
      aes(group = aq_category, ymin = y - sem, ymax = y)) +
    geom_linerange(
      stat = "identity",
      position = position_dodge(width = opts$dodge_w),
      size = 0.8, color = opts$blk,
      aes(group = aq_category, ymin = y, ymax = y + sem)) +
    ...
    theme(
      ...
      legend.key.height = unit(16, "pt"), legend.text.align = 1)
}
...
interaction_scatter <- function() {
  sre_plot_dta1 <- copy_dt() %>%
    .[
      , .(p = mean(recog_acc)),
      .(id, aq_category, aq_score, action_category, sub_sex)
    ]
  sre_plot_dta1 <- copy_dt(sre_plot_dta1, TRUE)
  sre_sub1_means <- sre_plot_dta1[, .(
    p = mean(p), aql = min(aq_score),
    aqu = max(aq_score)), .(Actions, AQ)]


  opts <- plot_setup()

  ggplot(sre_plot_dta1) +
    aes(x = aq_score, y = p, group = interaction(Actions, AQ)) +
    geom_vline(xintercept = 0, size = 0.25) +
    geom_hline(yintercept = 0.25, size = 0.25) +
    geom_segment(
      data = sre_sub1_means,
      aes(
        x = aql, y = p, yend = p,
        xend = aqu, color = Actions), linetype = 2,
      size = 0.3, show.legend = FALSE) +
    stat_smooth(aes(color = Actions),
      se = FALSE, method = "lm",
      fill = rgb(0.94, 0.94, 0.94, 0.5)) +
...
mcmc_checks <- function(stanreg) {
  ...
  plot((plot(stanreg, "trace", pars = "(Intercept)") %>%
    ...
    labs(title = "Target distribution coverage"))

  plot((plot(stanreg, "rhat_hist", binwidth = 5e-5) %>%
    gg_overrides() %>%
    change_geom_aes(1, colour = "white", size = opts$lwd)) +
    guides(colour = FALSE) + labs(title = "Chain consistency (Rhat)") +
    theme(axis.title.x = element_blank()))

  plot((plot(stanreg, "neff") %>% gg_overrides() %>% change_geom_aes(1, alpha = .5)) +
...
compare_rsquared <- function(ppd.full, ppd.alt = NULL, ppd.null = NULL) {
  r2_list <- lapply(list(
    `M1 (full)` = ppd.full, `M2 (alt)` = ppd.alt,
    `M3 (null)` = ppd.null), ppd_rsquared,
  yhat = ".y", y = "recog_acc")

  r2_data <- rbindlist(r2_list, idcol = "model")
  opts <- plot_setup()
  ggplot(r2_data) + aes(x = r_sq, y = model) +
    ggdistribute::geom_posterior(
      ci_width = 0.9, mirror = FALSE
    ) +
...
model_vs_data <- function(ppd_linpred, mname = "", ymax = NULL, n = 500, seed = NULL) {
  ...
  n <- min(n, s)
  r <- sample(s, n)
  p <- bayesplot::ppc_dens_overlay(y_obs$y, y_rep[r, ],
    n_dens = 512,
    adjust = 0.8, alpha = 0.15)
  ...
  (p %>%
    ...
    scale_x_continuous(breaks = seq(0, 1, .1)) + labs(title = mname) +
...
plot_setup <- function(font_size = NULL) {
  ...
  list(
    blk = gray(0.4), gry = gray(0.84),
    c_pal = c("#EDFADC", "#9EAD87", "#FDFFFA", "#A376AD", "#F4DCFA"),
    dodge_w = 0.75, lwd = 1 / 3)
}

gg_overrides <- function(gg, n_colors = 5) {
  suppressMessages(
    gg + scale_color_manual(values = viridisLite::viridis(n_colors)) +
