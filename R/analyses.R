#' @export
demographics_summary <- function(value, grps="aq_category", print=TRUE, ...) {
  demo_data <- copy_dt() %>%
    .[
...
time_delay_summary <- function(print=TRUE, ...) {
  ...
  dt1 <- dt[, .(e=mean(days_elapsed)), .(aq_category, id)] %>%
    .[, .(
      month_avg=mean(e) / 30.42, mean_days=mean(e), sd_days=sd(e),
      range=paste(range(e), collapse="-")
    ), .(aq_category)]

  dt2 <- dt[, .(e=mean(days_elapsed)), .(id)] %>%
    .[, .(
      aq_category="all", month_avg=mean(e) / 30.42, mean_days=mean(e),
...
acc_summary_actions <- function(print=TRUE, ...) {
  dt <- get_dt_by_cond_grps(c("action_category", "action"), omit=c("min", "max"))
...
acc_summary_subject <- function(print=TRUE, ...) {
  dt <- get_dt_by_cond_grps(c("aq_category"), "id", omit=c("min", "max"))
...
acc_summary_aq <- function(print=TRUE, ...) {
  dt <- get_dt_by_cond_grps(c("aq_category"), omit=c("min", "max"))
...
acc_summary_sex <- function(print=TRUE, ...) {
  dt_g <- get_dt_by_cond_grps(c("sub_sex"), "aq_category", omit=c("min", "max"))
  dt <- get_dt_by_cond_grps(c("sub_sex"), omit=c("min", "max"))
...
posterior_viewpoint_diff <- function(ppd, print=TRUE, ...) {
  ...
  rot_acc_diff <- multivariate_viewpoint(ppd) %>%
    ...
    melt(measure.vars=test_angles, value.name=".y", variable.name="Rotation")

  # posterior intervals and p vals
  results <- rot_acc_diff[, post_int_sre(.y), Rotation] %>%
    merge(rot_acc_diff[, .(`$p(D = 0)$`=prob_diff_from_null(.y)), Rotation],
      by="Rotation") %>%
    capital_ctr(suffix=" Diff.")
...
#' @export
posterior_rt_diff <- function(ppd, print=TRUE, ...) {
  gender_acc <- multivariate_gender(ppd)
...
posterior_action_acc_means <- function(ppd) {
  acc_means <- ppd[
    , .(.y=mean(.y), y=mean(recog_acc)), .(.sample, action, action_category)
  ]

  ordered_names <- acc_means[, mean(y), action] %>%
    .[order(V1), action]
...
posterior_aq_action_contrasts <- function(ppd) {
  posterior_aq_action_effects(ppd) %>%
    ...
    .[effect %in% c("smpl_low", "smpl_hig"), contrast := "simple acc"] %>%
    .[effect %in% c("cplx_low", "cplx_hig"), contrast := "complex acc"] %>%
    .[effect %in% c("action_main", "aq_main"), contrast := "Simple\ncontrasts"] %>%
    .[effect == "intx", contrast := "Interaction\ncontrast"] %>%
    .[
      effect %in% c("act_diff.hig", "act_diff.low", "aq_diff.smpl", "aq_diff.cplx"),
      contrast := "Simple\neffects"
    ] %>%
...
posterior_aq_action_intervals <- function(fx, print=TRUE, ...) {
  results <- fx[, post_int_sre(value), .(contrast, effect)] %>%
    merge(fx[, .(`Bayes *p*`=prob_diff_from_null(value)), .(contrast, effect)] %>%
      .[, c("", " ") := as.data.table(pval_format(`Bayes *p*`))],
...
contrast_effect_sizes <- function(ppd, print=TRUE, ...) {
  ...
  fx_size_cols <- c(
    "act_diff.hig", "act_diff.low", "aq_diff.cplx", "aq_diff.smpl",
    "action_main", "aq_main", "intx"
  )

  fx_size_tbl <- fx[, lapply(.SD, `/`, sd_g), .SDcols=fx_size_cols] %>%
    melt(measure.vars=fx_size_cols, value.name="d", variable.name="Effect size") %>%
