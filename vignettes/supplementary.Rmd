...
output:
  ...
  pdf_document:
    ...
    toc: yes
    pandoc_args: [
      "--csl", "apa.csl", "--include-in-header", "float.tex"
    ]
vignette: >
...
```{r pkg_debug, echo=FALSE, eval=FALSE, include=FALSE}
devtools::load_all(".")
...
```{r rerun_models, eval=FALSE, echo=FALSE, include=FALSE}
library(SRE)
sre_models(refit = FALSE, root_dir = "inst/extdata", debug = FALSE)
```

```{r build_supplement, echo = FALSE, eval=FALSE, include=FALSE}
devtools::load_all(".")
...
An updated version of this document and all supplementary materials can be found at the following address.

[`https://github.com/iamamutt/SRE`](https://github.com/iamamutt/SRE)

The data can be obtained without installing the package. Please review the `README.md` file for package installation instructions and how to use the exported R package functions.
...
```{r m1_mcmc, eval = TRUE, message=FALSE, fig.ncol=3, results='hold', fig.subcap=c("1", "2", "3"), fig.height=2, fig.width=2.15}
mcmc_checks(stanreg_full)
```

### Model 2 convergence diagnostics plots

```{r m2_mcmc, eval = TRUE, message=FALSE, fig.ncol=3, results='hold', fig.subcap=c("1", "2", "3"), fig.height=2, fig.width=2.15}
mcmc_checks(stanreg_alt)
```

### Model 3 convergence diagnostics plots

```{r m3_mcmc, eval = TRUE, message=FALSE, fig.ncol=3, results='hold', fig.subcap=c("1", "2", "3"), fig.height=2, fig.width=2.15}
...
```{r, pp_checks, message=FALSE, fig.ncol=3, results='hold', fig.subcap=c("1", "2", "3"), fig.height=2, fig.width=2.15}
...
To load or refit all the models, use the `sre_models` function. If `root_dir=NULL` and `refit=TRUE`, this will refit the default models and save all refitted models to a directory called `sre_fitted_models` in your OS specific user/home directory. If `refit=FALSE` then it will load the saved models from the package's external data folder.

```{r}
sre_models(refit = TRUE, root_dir = NULL)
```

If you specify a path for `root_dir`, all models can be saved and loaded from there. The `debug` option just runs a few iterations for each model.
